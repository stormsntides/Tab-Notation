...
function parseTabs(text){
  ...
  for(let z = 0; z < tokens.length; z++){
	...
	// stringTabs.unshift({stringName: "bt", tabs: beats, toTabString: toTabString});
  return stringTabs;

	function addColumn(char){
		stringTabs.forEach(function(st){
			st.tabs.push(char);
		});
	}
	...
	function addTabs(tabToken){
		...
		for(let t = 0; t < currentIndex.length; t++){
			...
			stringTabs[currentIndex[t]].tabs.push(value);
		...
		for(let st = 0; st < stringTabs.length; st++){
			if(!currentIndex.includes(st)){
				stringTabs[st].tabs.push(" ".repeat(numDigits));
			}
		}
	...
	// function addBeat(tabSize){
	// 	let beatSymbols = "!e&a";
	// 	let nextBeat = (beatSymbols.indexOf(beats[beats.length - 1]) + 1) % 4;
	// 	let b = (tabSize >= 2 ? " " : "") + beatSymbols[nextBeat];
	// 	beats.push(b);
	// }
}
